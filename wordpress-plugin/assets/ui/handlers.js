...
function handleConvertClick(event) {
  ...
  try {
    ...
    if (bricksJson.globalClasses.length > 0) {
      ...
      document.getElementById("otb-class-list").innerHTML = "<p>No classes were found or converted.</p>";
    }
...
function setupEventHandlers() {
  ...
  document.getElementById("otb-convert")?.addEventListener("click", handleConvertClick);
  
  // Handle load example button click
  document.getElementById("otb-load-example")?.addEventListener("click", handleLoadExampleClick);
  
  // Handle copy to clipboard button click
  document.getElementById("otb-copy")?.addEventListener("click", handleCopyClick);
  
  // Handle download JSON button click
  document.getElementById("otb-download")?.addEventListener("click", handleDownloadClick);
}
